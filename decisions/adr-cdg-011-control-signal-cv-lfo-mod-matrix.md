...
| 7 — effective-knob telemetry | `DiffusionFrame` fields reflect scheduler-read values, not the binding's static curve | Telemetry-honesty test: mutate `scheduler.config` via a fake walker, assert the captured frame matches the mutated (not original) value | **IN FORCE (read-path)** — issue #64 Phase 2, PR #69: `_FrameCollector` reads `entropy_bound`/`t_min`/`t_max` live off `scheduler.config` per callback; `test_effective_fields_reflect_mid_run_config_mutation` (`register_to_config` mid-run) is the honesty test. The write side (a config-mutating walker) remains `NOT-YET-IMPLEMENTED` — #35 R6, Phase 4 |
| 8 — same-in/same-out statelessness (F5) | two identical `run_diffusion(control_signals=...)` calls on one loaded model yield identical effective-knob telemetry | Same-in/same-out test on one loaded model (shared with ADR-CDG-010's pin-state clause) | **IN FORCE (effective-knob half)** — issue #64 Phase 2, PR #69: `TestEffectiveKnobTelemetryStatelessness` asserts identical effective-knob telemetry across two `run_diffusion` calls and that a mid-call `register_to_config` mutation never survives. The walker-state half stays `NOT-YET-IMPLEMENTED` — #35 R5/F5, Phase 4; ADR-CDG-008 Correction 1 — CDG-008 Phase-2 MCP state manager must never cache a scheduler |
