...
|---|---|---|---|
| 1 — declarative payloads only | `run_diffusion(control_signals=...)` accepts a payload, not a callable | `dgemma/ingress.py::validate_control_signals` (`ingress.py:82`) | **In force** — `tests/test_ingress.py::TestControlSignalsIngress::test_v1_callable_rejected` |
| 1 — schedule length == steps | a control signal shorter/longer than `num_inference_steps` is rejected, not truncated/padded | `dgemma/ingress.py::validate_control_signals` (`ingress.py:82`) | **In force** — `tests/test_ingress.py::TestControlSignalsIngress::test_v2_signal_shorter_than_steps_rejected`, `test_v2_signal_longer_than_steps_rejected` |
| 3 — units at binding, in-range | an out-of-declared-range control value, or an unknown target-knob name, is rejected at ingress | `dgemma/ingress.py::validate_control_signals` (`ingress.py:82`) | **In force** — `tests/test_ingress.py::TestControlSignalsIngress::test_v3_unknown_target_rejected`, `test_v6_signal_value_above_one_rejected`, `test_v6_signal_value_below_zero_rejected`, `test_v6_boundary_values_zero_and_one_are_accepted` |
| 4 — `scheduler.config`-only mutation, `num_inference_steps` excluded | the walker's mutable-target vocabulary never includes `num_inference_steps`; an attempt to bind it is rejected at ingress | `dgemma/ingress.py::validate_control_signals` (`ingress.py:82`), regression-anchored to #20 | **In force** — `tests/test_ingress.py::TestControlSignalsIngress::test_v4_num_inference_steps_target_rejected_with_20_anchored_message` |
| 5 — `t_min=t_max=v` exact-temperature mechanism | binding one signal to both `t_min` and `t_max` produces an exact per-step temperature with no anneal drift | Unit test over `anneal_temperature`/`EntropyBoundScheduler.step()` asserting `temperature == v` when `t_min == t_max == v` | **IN FORCE** — issue #64 Phase 4: `tests/test_control_signals.py::TestExactPerStepTemperature` binds one signal to both `t_min`/`t_max` and asserts `temperature == effective_t_min == effective_t_max` every walker-governed step. |
| 6 — walker prepares next step, capture records finished step | a captured frame's telemetry reflects the config that produced it, never the next step's | Ordered-composite test (shared fixture, R4) asserting capture reads pre-walker-mutation state each step | **IN FORCE** — issue #64 Phase 4: `dgemma/composite.py:StepEndComposite`'s `walker` slot runs LAST (after capture, cancellation, beta-rebuild, pin); `tests/test_step_end_composite.py::TestWalkerOrdering` proves the order structurally, `tests/test_control_signals.py::TestWalkerWritesThroughToNextFrame` proves a step-k write is visible only in frame k+1. |
| 7 — effective-knob telemetry | `DiffusionFrame` fields reflect scheduler-read values, not the binding's static curve | Telemetry-honesty test: mutate `scheduler.config` via a fake walker, assert the captured frame matches the mutated (not original) value | **IN FORCE (read + write path)** — read-path issue #64 Phase 2 (PR #69); write-path issue #64 Phase 4: `dgemma.participants.WalkerParticipant` is the real config-mutating walker `_FrameCollector`'s live read was always ready for. `tests/test_control_signals.py` exercises the full read+write loop end to end. |
| 8 — same-in/same-out statelessness (F5) | two identical `run_diffusion(control_signals=...)` calls on one loaded model yield identical effective-knob telemetry | Same-in/same-out test on one loaded model (shared with ADR-CDG-010's pin-state clause) | **IN FORCE** — effective-knob half issue #64 Phase 2 (PR #69); walker-state half issue #64 Phase 4: `tests/test_run_diffusion_statelessness.py::TestWalkerStatePerRun` — a fresh `WalkerParticipant` bound to that call's own scheduler every `run_diffusion` call, proven via identical telemetry across two identical calls and non-survival of a walker-written config into a subsequent no-`control_signals=` call. |
