...
implemented; only the beta-rebuild participant (ADR-CDG-010 Phase 5) remains
not yet implemented.
...
| `DiffusionFrame` extension discipline (additive-optional, heavy-field retention policy) | Optional-with-defaults fields; retention policy for heavy `DISTRIBUTION` | **In force (discipline + Tier 0 + Tier 1 + raw ids)** — #61 Phases P-A/P-B / ADR-CDG-014. `dgemma/types.py:DiffusionFrame` (`entropy`/`top_k_ids`/`top_k_weights`/`distribution`, all optional default `None`) + `CanvasTrace.raw_canvas_ids` (optional default `None`); `dgemma/loop.py:_FrameCollector.on_step_end` derives Tier 0 `entropy` from pre-pin `logits` (capture-first ordering, ADR-CDG-010), `_build_result` populates `raw_canvas_ids` before `excise_thought_channel`. **P-B adds:** `dgemma/payloads.py:CaptureSpec` (the `capture=` payload dataclass, minted per ADR-CDG-014 Decision 7 — `top_k`/`keep_frames`); `dgemma/ingress.py:validate_capture` gains `top_k` validation (non-negative int, in-vocab ceiling, `vocab_size=None` named degradation); `_FrameCollector.on_step_end` derives `top_k_ids`/`top_k_weights` from the same pre-pin `logits` Tier 0 reads (`topk` + top-k-slice-renormalized softmax), gated on `capture.top_k > 0` (default `0`/off, byte-identical to every pre-P-B run). `tests/test_frame_capture_discipline.py` (additive-optional construction, Tier 0/Tier 1 always-on/on-request, capture-pre-pin proof for both tiers), `tests/test_raw_canvas_ids.py` (pre-excision conservation, #9 EOS-in-thought-span probe), `tests/test_ingress.py` (`top_k` ingress register), `tests/test_capture_top_k_e2e.py` (real `run_diffusion(capture=CaptureSpec(top_k=k))` end to end against the R4 fixture). Tier 2 (`distribution` + budget) derivation remains `NOT-YET-IMPLEMENTED` — ADR-CDG-014 Phase P-C. |
| `KV_CACHE` payload types + ingress validation (V1-V6, fail-on-mismatch, both-token remedy messages) | `dgemma/types.py` (`KVCache`/`Provenance`/`EditOp` dataclasses, `CanvasTrace.injected_cache_provenance` additive-optional field) + `dgemma/kv_cache.py:validate_kv_cache_ingress`/`geometry_from_model`/`tokenizer_fingerprint` + `dgemma/loop.py:run_diffusion`'s `kv_cache=` door | **In force (types + validator + `run_diffusion` ingress/OUT-3 skeleton)** — ADR-CDG-012 / issue #62 Phases 1-2. `tests/test_kv_cache_types.py` (dataclass construction, illegal orphan state as data, `injected_cache_provenance` default), `tests/test_kv_cache_ingress.py` (V1-V6 happy path + every raise path, DV.3b both-token `match=`), `tests/test_kv_cache_run_diffusion.py` (`kv_cache=None` byte-identical to before; `kv_cache=<valid>` ingress-validates before scheduler/pipeline construction, stamps OUT-3, input-not-mutated; invalid cache rejected before any scheduler/pipeline object is built), `tests/test_run_diffusion_statelessness.py::TestKVCacheInjectionStatelessness` (same-in/same-out telemetry + provenance, no residual cache state across calls), `tests/conftest.py`'s `synthetic_kv_cache`/`fake_dgemma_model` fixtures (§L). Phase 2 lands ingress + OUT-3 trace-stamp ONLY — the decoder is NOT yet driven off an injected cache's tensors (that live drive body is gated on the ADR's real-weights de-risk smoke test, issue #62 Phase 4/Q-2). Surface nodes, socket mint, examples/workflows, DV.1/DV.2/DV.3a/DV.3c enforcement — `NOT-YET-IMPLEMENTED`, ADR-CDG-012 Phase 3. |
...
| ADR-CDG-012: MITM the AR/diffusion seam — `KV_CACHE` socket + `DGemmaEncode`/`DGemmaDenoise` | `KVCache`/`Provenance`/`EditOp` types (rule 4 `IDENTITY⊥ENVELOPE`); ingress fail-on-mismatch (rule 5); declarative `kv_cache=` door (rule 7) | `decisions/adr-cdg-012-mitm-seam-ar-diffusion-kv-cache.md` — Accepted (ratified 2026-07-13); Phases 1-2 (types + ingress + `run_diffusion` OUT-3 skeleton, issue #62) landed here, Phases 3-5 `NOT-YET-IMPLEMENTED` |
