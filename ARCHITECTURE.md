...
6. **The core is stateless across runs. The surface tier persists exactly two object classes: the model load (one per server process, replaced wholesale on `load_model`), and a bounded KV-cache registry scoped to the currently loaded model.** No mutable run-state survives a `run_diffusion` call. Two identical calls yield identical telemetry. `surfaces/mcp/state_manager.py`'s KV-cache registry (ADR-CDG-025) is entries keyed by opaque handle, evicted wholesale on `load_model`, identity-checked at resolve — a future third persisted class requires its own ADR amendment; this is not an open door. *(→ ADR-CDG-008, #35 R5, ADR-CDG-025 §1 · `STATELESS-CORE`)*
...
- **`surfaces/mcp/`** — Canonical surface (`server.py`, `state_manager.py`, `commands/{model,generate,encode}.py`). Thin adapter: unpack args → call one `dgemma.*` function (or, for `encode`, `StateManager.encode_into_registry`) → wrap the result. State manager persists the loaded model and (ADR-CDG-025) a bounded, model-scoped KV-cache handle registry — the two classes rule 6 sanctions, nothing else.
...
- **GGUF / llama.cpp backend (ADR-CDG-007)** — inference-only alternate backend, beside the drive seam, not through it. Rejected 2026-07-06; superseded by ADR-CDG-020 (**accepted 2026-08-06**, pinned-upstream-PR posture; pin = #24427 @ `dd0cf04`), inference-only-secondary per INV-5.
