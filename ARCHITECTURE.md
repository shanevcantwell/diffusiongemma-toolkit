...
| Single hardcoded callback binding; no composition / ordering / exception layer | Rule 7 — five expansion participants want the slot with ordering semantics | `dgemma/loop.py:582` (`callback_on_step_end=collector.on_step_end`) | `NOT-YET-IMPLEMENTED` — #35 R1 |
...
| Composition ordering (capture pre-pin; β-rebuild before pin; pin last writer) | Ordered-composite test over the shared fake-pipeline fixture | `NOT-YET-IMPLEMENTED` — #35 R1 (over R4's fixture). ADR-CDG-010. |
